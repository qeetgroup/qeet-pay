...
`/pay/{code}` resolves a payment link and lets a payer pay it with **no merchant API key** — it talks to
...
- **API docs** — springdoc OpenAPI 3.1 at `/v3/api-docs` (226 paths, incl. the public `/v1/checkout/**`),
  ...
  [api/openapi/](api/openapi/) — `payments`/`payouts`/`billing`/`tax`/`commerce`/`risk`/`platform.yaml`
